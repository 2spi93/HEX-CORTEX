...
class LocalKnowledgeIndex:
    ...
    def __init__(self, entries: list[IndexEntry] | None = None) -> None:
        ...
        self._lowered_tags: dict[str, frozenset[str]] = {}
    ...
    def exact_search(
        ...
        for entry_id, entry in self._entries.items():
            if required and not required.issubset(self._lowered_tags[entry_id]):
                continue
            if normalized_query in self._haystacks[entry_id]:
                hits.append(entry)
        return hits
    ...
    def lexical_scores(
        ...
        for entry_id, entry in self._entries.items():
            ...
            for term in query_terms:
                ...
                if tf == 0:
                    continue
                score += (tf / (tf + 1.5)) * self._idf(term)
    ...
    def _rebuild_if_needed(self) -> None:
        ...
        for entry_id, entry in self._entries.items():
            ...
            self._lowered_tags[entry_id] = frozenset(tag.lower() for tag in entry.tags)
            self._haystacks[entry_id] = f"{entry.title}\n{entry.text}".lower()
