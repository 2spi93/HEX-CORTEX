...
def build_world_model_run_plan(
    *,
    ...
    return {
        ...
        "blockers": blockers,
        "next_action": "execute_world_model_run_in_sandbox" if allowed else "repair_world_model_run_plan",
    }
...
def evaluate_world_model_candidate(
    *,
    ...
    for metric in required:
        ...
        signed_gain = (value - base) / denominator if higher_is_better else (base - value) / denominator
        deltas[metric] = round(signed_gain, 8)
        if signed_gain < -max_regression_fraction:
            regression_metrics.append(metric)
        if signed_gain >= minimum_improvement_fraction:
            improvement_metrics.append(metric)
    promotion_allowed = not regression_metrics and bool(improvement_metrics)
    stable = {
    ...
    return {
        ...
        "next_action": "prepare_candidate_promotion" if promotion_allowed else "retain_baseline",
    }
